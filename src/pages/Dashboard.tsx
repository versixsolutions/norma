...
export default function Dashboard() {
  ...
  const loadUnifiedFeed = useCallback(async () => {
    ...
    try {
      const [comunicados, despesas, ocorrencias, votacoes, faqs, documentos] =
        await Promise.all([
          fetchData(
            "comunicados",
            supabase
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 pb-24 md:pb-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6" data-tour="dashboard-stats">
        ...
        <DashboardCard
          ...
          label="Assembleias"
          kpi={
            stats.votacoes.ativas > 0 ? `${stats.votacoes.ativas} ativas` : ""
          }
        ...
        <DashboardCard
          ...
          label="Ocorrências"
          kpi={
            stats.ocorrencias.abertas > 0
              ? `${stats.ocorrencias.abertas} abertas`
              : ""
          }
          ...
          <div className="space-y-0 divide-y divide-gray-50">
            {updates.length === 0 ? (
              ...
              updates.map((item) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-lg shadow-sm ${item.bgColor}`}
                  >
                    {item.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-sm font-bold truncate ${item.color}`}>
                      {item.isPinned && (
                        <span className="mr-1.5 text-[9px] bg-red-100 text-red-600 px-1.5 rounded border border-red-200 align-middle">
                          FIXO
                        </span>
                      )}
                      {item.title}
...
function DashboardCard({
  ...
  return (
    ...
    >
      ...
      {kpi && (
        <span
          className={`text-[10px] font-semibold mt-1 truncate w-full px-1 ${accentColor || (alert ? "text-red-500" : "text-gray-400")}`}
        >
          {kpi}
        </span>
      )}
