...
export default function Dashboard() {
  ...
  async function loadBanner() {
    ...
    const { data } = await supabase
      ...
      .eq('active', true)
      .limit(1)
      .order('created_at', { ascending: false }) // Ou random() se preferir
    
    if (data && data.length > 0) {
      setBanner(data[0])
      // Contar visualização (sem await para não travar)
  ...
  async function loadUnifiedFeed() {
    ...
    try {
      ...
      comunicados?.forEach((c: any) => { if (c.title.startsWith('Novo Documento:')) return; const isUrgent = c.priority >= 3; newUpdates.push({ id: `com-${c.id}`, type: 'comunicado', title: isUrgent ? `URGENTE: ${c.title}` : c.title, description: c.content, date: c.published_at || c.created_at, icon: isUrgent ? '📢' : '📌', color: isUrgent ? 'text-red-600' : 'text-blue-600', bgColor: isUrgent ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-100', link: '/comunicados', isPinned: isUrgent }) })
      documentos?.forEach((d: any) => { newUpdates.push({ id: `doc-${d.id}`, type: 'documento', title: d.title || d.metadata?.title || 'Novo Documento', description: `Adicionado à Biblioteca (${d.metadata?.category || 'Geral'})`, date: d.created_at, icon: '📂', color: 'text-indigo-600', bgColor: 'bg-indigo-50 border-indigo-100', link: '/biblioteca' }) })
      despesas?.forEach((d: any) => { newUpdates.push({ id: `desp-${d.id}`, type: 'despesa', title: 'Nova Despesa', description: `${d.description} - ${formatCurrency(d.amount)}`, date: d.created_at, icon: '💰', color: 'text-green-600', bgColor: 'bg-white border-gray-100', link: '/transparencia' }) })
      ocorrencias?.forEach((o: any) => { newUpdates.push({ id: `oco-${o.id}`, type: 'ocorrencia', title: `Ocorrência: ${o.status}`, description: o.title, date: o.created_at, icon: '🚨', color: 'text-orange-600', bgColor: 'bg-white border-gray-100', link: '/ocorrencias' }) })
      votacoes?.forEach((v: any) => { const isActive = new Date(v.end_date) > new Date(); newUpdates.push({ id: `vot-${v.id}`, type: 'votacao', title: isActive ? 'Nova Assembleia' : 'Votação Encerrada', description: v.title, date: v.created_at, icon: '🗳️', color: 'text-purple-600', bgColor: 'bg-purple-50 border-purple-100', link: '/votacoes' }) })
      faqs?.forEach((f: any) => { newUpdates.push({ id: `faq-${f.id}`, type: 'faq', title: 'Dúvida Respondida', description: f.question, date: f.created_at, icon: '❓', color: 'text-cyan-600', bgColor: 'bg-white border-gray-100', link: '/faq' }) })
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 py-4 pb-24 md:pb-8">
      ...
      {/* NOVA GRID COMPACTA 
         - grid-cols-3: 3 itens por linha
         - gap-2: espaçamento reduzido
      */}
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        {/* Card 6: Biblioteca (Ou outro módulo futuro) */}
