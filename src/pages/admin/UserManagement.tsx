...
export default function UserManagement() {
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      if (error) throw error;
      setUsers((prev) =>
        prev.map((u) => (u.id === id ? { ...u, role: "morador" } : u)),
      );
  ...
  async function handleDeleteUser(id: string) {
    if (
      !confirm(
        "ATENÇÃO: Isso excluirá PERMANENTEMENTE o usuário e todos os seus dados (login, perfil, histórico). Tem certeza?",
      )
    )
      return;
    ...
    try {
      ...
      if (
        confirm(
          "A exclusão completa falhou. Deseja forçar a remoção apenas do painel (o login pode permanecer)?",
        )
      ) {
  ...
  function openEditModal(targetUser: UserData) {
    if (!isAdmin && targetUser.role === "admin") {
      toast.error(
        "Apenas Super Administradores podem editar outros Administradores.",
      );
      return;
    }
  ...
  async function handleSaveEdit(e: React.FormEvent) {
    ...
    if (!isAdmin && editingUser.role === "admin") {
      toast.error(
        "Você não tem permissão para promover usuários a Administrador.",
      );
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Administre permissões e cadastros deste condomínio.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Nenhum registro encontrado para este condomínio."
        />
      ) : (
                          ...
                          {!isAdmin && user.role === "admin" ? (
                            ...
                            <>
                              <button
                                ...
                                className="text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded text-sm font-medium transition"
                              >
                                Editar
                              </button>
                              <button
