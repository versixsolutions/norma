  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState icon="📢" title="Mural vazio" description="Nenhum comunicado publicado ainda." />
      ) : (
        <div className="grid gap-4">
          {comunicados.map((item) => (
            <div key={item.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition relative group">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${
                      item.priority >= 3 ? 'bg-red-50 text-red-600 border-red-200' : 'bg-blue-50 text-blue-600 border-blue-200'
                    }`}>
                      {item.type}
                    </span>
