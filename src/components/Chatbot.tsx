...
export default function Chatbot({ isOpen, onClose }: ChatbotProps) {
  ...
  async function handleSendMessage(e: React.FormEvent | null, textOverride?: string) {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        text: 'Estou com dificuldade de conexão no momento. Tente novamente em instantes.',
        sender: 'bot',
        timestamp: new Date(),
        isError: true
      }])
  ...
  return (
    <div className="fixed bottom-4 right-4 left-4 md:left-auto md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden z-50 h-[500px] animate-fade-in-up">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-4">
        ...
        {isTyping && <div className="flex justify-start"><div className="bg-white border border-gray-200 p-3 rounded-2xl rounded-bl-none flex gap-1 items-center shadow-sm"><span className="text-[10px] text-gray-400 mr-2 font-medium">Norma pensando...</span><span className="w-1.5 h-1.5 bg-primary/60 rounded-full animate-bounce"></span><span className="w-1.5 h-1.5 bg-primary/60 rounded-full animate-bounce delay-100"></span><span className="w-1.5 h-1.5 bg-primary/60 rounded-full animate-bounce delay-200"></span></div></div>}
