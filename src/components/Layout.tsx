...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
          ...
          {visibleMobileItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span
                    className={`text-xl mb-0.5 ${active ? "scale-110" : ""} transition-transform`}
                  >
                    {item.icon}
                  </span>
                  <span
                    className={`text-[9px] font-medium truncate w-full text-center ${active ? "font-bold" : ""}`}
                  >
                    {item.label}
                  </span>
                  {item.badge !== undefined && item.badge > 0 && (
      ...
      <main
        id="main-content"
        className="pb-24 md:pb-8 animate-fade-in flex-1"
        tabIndex={-1}
      >
        <Outlet />
      </main>
